...
namespace StandardizedLogging
{
	...
	void CStandardizedLoggerImpl::PushMainLoopStart(const int nProductCount, const int nMainThreadIdx)
	{
		PushProcessLog(nProductCount, _T("Null"), EProcessLogThread::MainThread, nMainThreadIdx, m_strVisionSystemMinorName, EPreTag::None, EPostTag::MainLoopStart);
	}

	void CStandardizedLoggerImpl::PushMainLoopEnd(const int nProductCount, const CString& strProductId, const int nMainThreadIdx)
	{
		PushProcessLog(nProductCount, strProductId, EProcessLogThread::MainThread, nMainThreadIdx, m_strVisionSystemMinorName, EPreTag::None, EPostTag::MainLoopEnd);
	}
	...
	void CStandardizedLoggerImpl::PushProcessLog(const int nProductCount, const CString & strProductId, const EProcessLogThread eLogThread, const int nThreadIdx, const CString & strLogContent, const EPreTag ePreTag, const EPostTag ePostTag)
	{
	...
	void CStandardizedLoggerImpl::PushAlarmLog(const int nProductCount, const CString & strProductId, const CString & strLogContent)
	{
		CTime curTime = CTime::GetCurrentTime();
		CString strLogTime = GetFormattedTime(curTime);
		const auto pLogItem = std::make_shared<SLogItem>();
	...
	void CStandardizedLoggerImpl::PushResultLog(const int nProductCount, const CString & strModuleId, const CString & strCellId, const StandardizedLogging::EResultValue eResultValue, const CString & strImgPath, const std::vector<CString>& vctLogs)
	{
		CTime curTime = CTime::GetCurrentTime();
		CString strLogTime = GetFormattedTime(curTime);
		const auto pLogItem = std::make_shared<SLogItem>();
	...
	void CStandardizedLoggerImpl::PushSystemLog(const int nProductCount, const CString & strProductId, const StandardizedLogging::ESystemLogThread eLogThread, const CString & strLogContent, const StandardizedLogging::EPreTag ePreTag, const StandardizedLogging::EPostTag ePostTag)
	{
		CTime curTime = CTime::GetCurrentTime();
		CString strLogTime = GetFormattedTime(curTime);
		const auto pLogItem = std::make_shared<SLogItem>();
	...
	{
		m_bThreadRunning = false;
		const int nTenSecsInMilliseconds = 100000;
		WaitForSingleObject(m_pSaveStandardLogThread->m_hThread, nTenSecsInMilliseconds);
		CSingleLock lock(&m_csQueue);
